...
var options = {
  ...
  exec: function exec(resolve) {
    var apmWhich = shell.exec('which apm', { silent: true });
    if (apmWhich.output.trim() !== '') {
      if (userConfig.apmPackages.length > 0) {
        ...
        var packages = userConfig.apmPackages.join(' ');
        shell.exec('apm install ' + packages);
        winston.success('apm packages installed');
      } else winston.info('no packages to install');
    } else winston.error('Atom IDE not installed');
    resolve();
  }
};
